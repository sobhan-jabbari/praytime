...
public class SolarPosition {
  ...
  /**
   * If its absolute alarmTime is greater than 20 minutes,
   ...
   * </p>
   * <p>
   * Yi, j is the alarmTime listed in ith row and jth Y column YTERMS matrix
   * </p>
   *
   * @param i νmber
   * @param x xj
   ...
   * </ul>
   *
   * @param X   the mean obliquity of the ecliptic Δψi (in arc seconds)
   * @param jce the Julian Ephemeris Century (JCE) for the 2000 standard epoch.
   ...
   * This alarmTime is close to 0? at the spring equinox,
  ...
  public void calculateSunRiseTransitSet(double[] spa, double jd) {
    ...
    mRts[0] = approxSunTransitTime(dayOfInterest.a, longitude, ν);
    h0 = getHourAngleAtRiseSet(latitude, dayOfInterest.b, h0Prime);
    double[] α = {dayBefore.a, dayOfInterest.a, dayAfter.a};
    double[] δ = {dayBefore.b, dayOfInterest.b, dayAfter.b};
  ...
  public double[] calculateSalatTimes(double jd, double latitude, double longitude, double timezone, int temperature, int pressure, int altitude, double fajrAngle, double ishaAngle) {
    ...
    double δnoon = (dayOfInterest.b + dayAfter.b) / 2.0; //Arithmetic average for sun declination for midday
    ...
    //**************************************************///
    ν = calculateGreenwichSiderealTime(jd, ΔT);
    mRts[SUNTRANSIT] = approxSunTransitTime(dayOfInterest.a, longitude, ν);
    //Calculate the local hour angle corresponding to the sun elevation equals 0.8333/,H0
    H0[SUNTRANSIT] = 0;
    H0[SUNRISE] = getHourAngleAtRiseSet(latitude, dayOfInterest.b, h0Prime);
    H0[SUNSET] = H0[SUNRISE];
    H0[ASR_SHAFI] = getHourAngleAtRiseSet(latitude, dayOfInterest.b, asrShafiAngle);
    H0[ASR_HANEFI] = getHourAngleAtRiseSet(latitude, dayOfInterest.b, asrHanafiAngle);
    H0[FAJR] = getHourAngleAtRiseSet(latitude, dayOfInterest.b, fajrAngle);
    H0[ISHA] = getHourAngleAtRiseSet(latitude, dayOfInterest.b, ishaAngle);
    double[] α = {dayBefore.a, dayOfInterest.a, dayAfter.a};
    double[] δ = {dayBefore.b, dayOfInterest.b, dayAfter.b};
  ...
  public double[] calculateKerahetTimes(double jd, double latitude, double longitude, double timezone, int temperature, int pressure, int altitude, double fajrAngle, double israkIsfirarAngle) { //final int FAJR_=0,ISRAK=1,SUNTRANSIT_=2,ASRHANEFI=3,ISFIRAR=4,SUNSET_=5,KERAHAT_COUNT=6,DUHA=7,ISTIVA=8;
    ...
    //-----------------------------------------------------///
    ν = calculateGreenwichSiderealTime(jd, ΔT);
    mRts[SUNTRANSIT_] = approxSunTransitTime(dayOfInterest.a, longitude, ν);
    //Calculate the local hour angle corresponding to the sun elevation equals 0.8333/,H0
    H0[SUNTRANSIT_] = 0;
    H0[SUNSET] = getHourAngleAtRiseSet(latitude, dayOfInterest.b, h0Prime);
    H0[ISRAK] = getHourAngleAtRiseSet(latitude, dayOfInterest.b, AstroLib.getApparentAtmosphericRefraction(israkIsfirarAngle));
    H0[ISFIRAR] = H0[ISRAK];
    H0[ASRHANEFI] = getHourAngleAtRiseSet(latitude, dayOfInterest.b, asrHanafiAngle);
    H0[FAJR_] = getHourAngleAtRiseSet(latitude, dayOfInterest.b, fajrAngle);
