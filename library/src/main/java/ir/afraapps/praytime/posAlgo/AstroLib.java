...
import ir.afraapps.praytime.phaseEvents.MoonPhases;
...
public class AstroLib {
   ...
   */
  static double getApparentAtmosphericRefraction(double angle) {
    double R = 1 / (Math.tan(Math.toRadians(angle + 7.31 / (angle + 4.4)))) + 0.001351521723799;
    return R / 60;
  }
   ...
   * @result corection for altitude angle in Minutes
   */
  public static double getAtmosphericRefraction(double angle) {
    double R = 1.02 / (Math.tan(Math.toRadians(angle + 10.3 / (angle + 5.11))))
      + +0.0019279;
    return R / 60;
  }
   ...
   * @return Weather Correction Coefficent unitless
   */
